...
int main(){
//	MainRenderThread_Primitive::init(width, height);
//	lighting_main(); // phong shading and gouraud shading
//	pst_main(); // point rendering and triangle rendering
//	cookTorance(); // cook-torrance lighting model
//	MainRenderThread_Primitive::instance().waitForExit();

	particle_main(); // particle system with GPU fluid simulation
	return 0;
}
void particle_main(){
	RenderThread_ECS::init(width, height);
	auto particleEntity = std::make_shared<Entity>();
	particleEntity->addComponent<TransformComponent>(
...
//	MainRenderThread_Primitive::add(std::move(gouraud));
//	MainRenderThread_Primitive::add(std::move(phong));
//}
//int pst_main(){
//	MainRenderThread_Primitive::init(width, height);
...
////	MainRenderThread_Primitive::add(std::move(ptr));
...
//
//	std::string projectRoot = getProjectPath();
//	std::string cookTorranceShader[] = {
//			projectRoot + "/Shader/Ball/CookTorranceLighting/CookTorrance.frag",
//			projectRoot + "/Shader/Ball/CookTorranceLighting/CookTorrance.vert"
//	};
//
//	auto position = Eigen::Vector4f(0, 0, 0, 1);
//	auto cookTorrance = std::make_unique<BallRender>(1.0f, cookTorranceShader[1], cookTorranceShader[0]);
//	cookTorrance->setPosition(position);
//	cookTorrance->setCamera(camera);
//
//	MainRenderThread_Primitive::add(std::move(cookTorrance));
//}